   ...
   u8 pps_state)
{
  ...
  if (dolby_vision_on && (last_dolby_vision_ll_policy != dolby_vision_ll_policy ||
    ...
    if (is_meson_box() || is_meson_tm2_stbmode() || is_meson_sc2()) {
      ...
      /* POST matrix needed when Core3 outputs RGB:
       *   - DV LL mode: Core3 outputs RGB
       *   - HDR10 mode: Core3 outputs RGB, but NOT when VP forces IPT bypass */
  ...
  /* Skip when VP forces Core3 to IPT bypass - output is not RGB */
