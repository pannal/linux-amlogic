 ...
 * Kept for potential future use with Core3 mode 0x00 bypass. */
   ...
   int vsize)
{
  ...
  if (xbmc_dv_vp != 0 && xbmc_dv_vp_tm > 3) {
    /* VP: Keep library's a2b (RGB→LMS) and c2d (LMS→IPT) intact.
     * Core3 mode 0x02 applies d2c+b2a (inverse of a2b+c2d, cancels out),
     * then OETF (linear→PQ on linear RGB — correct place for PQ), then
     * POST matrix (RGB→YCbCr).  Only g_2_l needs fixing: replace the
     * library's DV tone-map curve with standard gamma 2.2 degamma. */

    /* DIAG: log library values */
  ...
  if (set_lut || reset || force_set_lut) {
    ...
    if (xbmc_dv_vp != 0 && xbmc_dv_vp_tm > 3) {
      ...
      /* DIAG: log overridden g_2_l values */
      if (diag_frame <= 3 || diag_frame == 60 || diag_frame == 120) {
   ...
   u8 pps_state)
{
  ...
  /* DIAG: Core3 state for VP_TM>3 */
  if (xbmc_dv_vp != 0 && xbmc_dv_vp_tm > 3) {
    static int c3_diag;
    c3_diag++;
    if (c3_diag <= 3 || c3_diag == 60) {
      pr_info("DV_DIAG C3 frame=%d cur_dv_mode=%d mode_written=%d\n",
        c3_diag, cur_dv_mode, cur_dv_mode);
