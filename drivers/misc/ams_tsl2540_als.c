	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%10d,%10d,%10d,%10d,%10d", &d_factor,
			&ch0_coef1, &ch1_coef1, &ch0_coef2, &ch1_coef2) != 5)
		return -EINVAL;
...
static ssize_t als_lux_400_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct tsl2540_chip *chip = dev_get_drvdata(dev);
	struct tsl2540_i2c_platform_data *pdata = chip->pdata;
	int lux_400 = pdata->lux400_lux;

	return snprintf(buf, PAGE_SIZE, "%d\n", lux_400);
}

static ssize_t als_lux_400_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t size)
{
	long lux_400;
	...
	if (kstrtol(buf, 10, &lux_400))
		return -EINVAL;

	if (lux_400 < ALS_MIN_LUX)
		return -EINVAL;
...
static DEVICE_ATTR(als_lux_400, 0644, als_lux_400_show,
		   als_lux_400_store);

static ssize_t als_lux_20_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct tsl2540_chip *chip = dev_get_drvdata(dev);
	struct tsl2540_i2c_platform_data *pdata = chip->pdata;
	int lux_20 = pdata->lux20_lux;

	return snprintf(buf, PAGE_SIZE, "%d\n", lux_20);
}

static ssize_t als_lux_20_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t size)
{
	long lux_20;
	...
	if (kstrtol(buf, 10, &lux_20))
		return -EINVAL;

	if (lux_20 < ALS_MIN_LUX)
		return -EINVAL;
...
static DEVICE_ATTR(als_lux_20, 0644, als_lux_20_show,
		   als_lux_20_store);
#endif
...
static struct attribute *tsl2540_als_attributes[] = {
	...
	&dev_attr_als_lux_400.attr,
	&dev_attr_als_lux_20.attr,
#endif
#ifdef LUX_DBG
